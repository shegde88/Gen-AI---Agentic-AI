...
### Quality Fix — Historical World Cup articles appearing as citations for 2026 queries
Answers about 2026 training camps and venues were citing "2010 FIFA World Cup" and similar historical articles as sources. Root cause: the historical articles score above `CITATION_MIN_SCORE = 0.40` because they share training-camp vocabulary ("base camp", "training facility") with the query. The relevance score alone isn't enough to distinguish era. Fix: added era suppression to `_collect_citations()` — if at least one qualifying citation is 2026-specific (`year == "2026"` or title contains "2026"), historical year-specific articles are excluded from the sources panel. Team/player articles (`year = "all"`) always display regardless. Historical articles still appear correctly when no 2026 source qualifies (e.g. "Who won the 1994 World Cup?" correctly cites "1994 FIFA World Cup").
